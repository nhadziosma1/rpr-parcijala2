...
public class GeografijaDAO
{
    ...
    private GeografijaDAO()
    {
        ...
        try
        {
            glavniGradUpit = conn.prepareStatement("SELECT grad.id, grad.naziv, grad.broj_stanovnika, grad.drzava, grad.nadmorska_visina" +
        ...
        try
        {
            ...
            promijeniGradUpit = conn.prepareStatement("UPDATE grad SET naziv=?, broj_stanovnika=?, drzava=?, nadmorska_visina=? " +
                                                    " WHERE id=?");
        }
        catch (SQLException e)
        {
    ...
    {
        Grad grad = new Grad(rs.getInt(1), rs.getString(2), rs.getInt(3), null, rs.getInt("nasmorska_visina"));
